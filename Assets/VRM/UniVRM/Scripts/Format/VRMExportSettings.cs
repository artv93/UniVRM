...
#pragma warning restore 0618
        ...
        public void RuntimeExport(string path)
        {
            List<GameObject> destroy = new List<GameObject>();
            try
            {
                RuntimeExport(path, destroy);
            }
            finally
            {
                foreach (var x in destroy)
        ...
        void RuntimeExport(string path, List<GameObject> destroy)
        {
            var target = Source;
            ...
            if (PoseFreeze)
            {
                ...
                var smrSource = Source.GetComponentsInChildren<SkinnedMeshRenderer>();
                var smrDest = normalized.Root.GetComponentsInChildren<SkinnedMeshRenderer>();
                for (uint i = 0; i < smrDest.Length; ++i)
                {
                    smrDest[i].localBounds = new Bounds(smrSource[i].localBounds.center, smrSource[i].localBounds.size);
                }
